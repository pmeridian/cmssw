...
FTLUncalibratedRecHit::FTLUncalibratedRecHit() :
  amplitude_(-1.f), time_(-1.f), timeError_(-1.f), id_(DetId()), row_(0), column_(0), flags_(std::numeric_limits<unsigned char>::max()) { }
...
FTLUncalibratedRecHit::FTLUncalibratedRecHit(const DetId& id, uint8_t row, uint8_t column, float ampl, float time, float timeError, unsigned char flags) :
